...
func _draw() -> void:
	...
	text(Vector2(17,316),"KICK BOOTS / READY" if world.player.wall_unlocked and world.player.state.wall_kick_available else "KICK BOOTS / SPENT" if world.player.wall_unlocked else "KICK BOOTS / OFFLINE",8,MINT if world.player.wall_unlocked else Color("6d868a"))
	...
	if world.paused:
		...
		text(Vector2(181,157),"SPACE / Z     Jump. Boots: one wall kick per landing.",11)
		text(Vector2(181,177),"S + J     Recoil lift. Ammo refills ONLY on landing.",11)
...
func _title() -> void:
	...
	text(Vector2(42,284),"Earn your gun, magazines, kick boots and vector thruster.",10,Color("9aafab"))
