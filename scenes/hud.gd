...
func _draw() -> void:
	...
	for i in range(5):
		draw_rect(Rect2(500+i*21,20,15,6),MINT if i < world.player.health else Color("344750"))
	...
	text(Vector2(291,40),"TO THE SURFACE",7,MINT)
	# Quiet altitude rail along the edge of the viewport.
	draw_rect(Rect2(621,77,2,200),Color("354d53"))
	var progress := clampf((472-world.player.position.y)/1976,0,1)
	draw_rect(Rect2(619,274-progress*197,6,4),MINT)
	text(Vector2(17,316),"AIR JUMP" if world.player.jump_unlocked else "AIR JUMP / OFFLINE",8,MINT if world.player.jump_unlocked else Color("6d868a"))
	text(Vector2(17,328),"VECTOR DRIVE" if world.player.dash_unlocked else "VECTOR DRIVE / OFFLINE",8,MINT if world.player.dash_unlocked else Color("6d868a"))
	text(Vector2(17,348),"A D  MOVE     SPACE  JUMP     MOUSE / J  FIRE     S + J  RECOIL     SHIFT  DASH",9,Color("a1b7b1"))
	text(Vector2(535,348),"ESC PAUSE   M " + ("UNMUTE" if world.muted else "MUTE"),8,Color("a1b7b1"))
	...
	if world.paused:
		...
		text(Vector2(181,157),"SPACE / Z     Jump. Hold longer to jump higher.",11)
		text(Vector2(181,177),"S + J             Fire down for recoil lift.",11)
		text(Vector2(181,197),"W A S D + SHIFT     Directional dash after unlock.",11)
		text(Vector2(181,217),"R     Return to your latest signal anchor.",11)
		text(Vector2(226,246),"ESC / P TO RESUME",12,MINT)
	if world.finished:
...
func _title() -> void:
	...
	text(Vector2(42,250),"A D / ARROWS  Move       SPACE  Jump",10)
	text(Vector2(42,267),"MOUSE + CLICK  Aim & shoot       S + J  Recoil lift",10)
	text(Vector2(42,284),"Recover suit modules to unlock air jump and dash.",10,Color("9aafab"))
	text(Vector2(42,337),"CHAPTER ONE  /  FIRST LIGHT",9,MINT)
	text(Vector2(493,337),"HEADPHONES ON",9,Color("a8bcba"))
